  ...
  <div class="list-item">
    <div class="list-item-header van-hairline--bottom">
      <div class="list-item-header-hd">
        <span class="title">Order：{{ item.orderNumber }}</span>
      </div>
      <div :class="['list-item-header-state', item.status !== -1 ? 'text-primary-color' : '']">
        {{ orderStatus(item.statusStr) }}
      </div>
    </div>
    <div class="list-item-body" @click="onOrderClicked(item)">
      ...
      <div v-if="item.goodsNumber > 1" class="list-item-more">view all({{ item.goodsNumber }}) item</div>
      <div class="list-item-total van-hairline--top">
        <span class="list-item-total-number">{{ item.goodsNumber }}Item</span>
        <div class="list-item-total-price">
          <span class="list-item-total-price-label"> {{ item.status === 0 ? 'To Pay：' : 'Paid：' }}</span>
    ...
    <div class="list-item-footer van-hairline--top">
      <template v-if="item.status === -1 || item.status === 3 || item.status === 4">
        <van-button class="list-item-action-btn" round @click.stop="onOrderDelete(item, index)"> Delete </van-button>
      </template>
      <template v-if="item.status === 0">
        <van-button class="list-item-action-btn" round plain @click.stop="onOrderCancel(item)"> Cancel </van-button>
