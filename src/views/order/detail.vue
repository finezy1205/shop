    ...
    <van-pull-refresh v-model="isLoading" :disabled="pullRefreshDisabled" @refresh="onRefresh">
      <div class="header">
        <div :class="['order-status', `order-status--${orderInfo.status}`]">
          <div class="order-status-title">{{orderStatus(orderInfo.statusStr)}}</div>
          <template v-if="orderInfo.status === 0 && closeTime > 0">
            <div class="order-status-desc">
              Please Pay in<van-count-down
                class="count-down"
                :time="closeTime"
                format="mm 分 ss 秒"
        ...
        <div class="order-step" @click="onStepsOpen">
          <span class="order-step-label">Track Package</span>
        ...
        <div class="address van-hairline--top">
          <div class="address-hd">
            <div class="address-inner">
              ...
              <div class="address-inner-title">Receiver：{{ logistics.linkMan }}</div>
              <div class="address-inner-title">{{ logistics.mobile }}</div>
            </div>
            <div class="address-inner-bottom">Address：{{ logistics.address }}</div>
          </div>
        </div>
        <van-cell title="Tracking Details" class="cell">
      ...
      <div class="section">
        <div class="section-header van-hairline--bottom">
          ...
          <span class="section-header-title">Order</span>
        </div>
        <div class="list">
...
<!--          </div>-->
          <div class="order-no-p">
            Payment Method：
            <span class="order-no-p-value">Online Payment</span>
          </div>
          <div v-if="orderInfo.isNeedLogistics" class="order-no-p">
            Shipping Info：
            <span class="order-no-p-value"> Standard Delivery</span>
          </div>
        </div>
      </div>
    </van-pull-refresh>
    ...
    <div class="action-bar-wrap">
      <div class="action-bar">
        ...
        <template v-if="orderInfo.status === -1 || orderInfo.status === 3 || orderInfo.status === 4">
          <van-button class="action-bar-btn" round @click.stop="onOrderDelete(orderInfo.id)"> Delete </van-button>
        </template>
        <template v-if="orderInfo.status === 0">
          <div class="action-bar-hd">
            <span class="action-bar-total">Total：</span>
        ...
        <template v-if="orderInfo.status === 2">
          <van-button class="action-bar-btn" round @click.stop="onOrderDelivery(orderInfo.id)">Received</van-button>
        </template>
        <template v-if="orderInfo.status === 3">
          <van-button class="action-bar-btn" round @click.stop="onOrderReputation">To Review</van-button>
        </template>
        <!-- ▲ 操作按钮组 -->
      </div>
    </div>
