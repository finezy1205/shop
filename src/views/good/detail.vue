...
const goodDeliveryTitle = computed(() => {
  if (unref(basicInfo).logisticsId) {
    return `${unref(logistics).isFree ? 'Free Delivery' : '不包邮'}`;
  } else {
    return `无需配送`;
  }
});
...
<template>
  <div class="container">
    ...
    <div class="stock van-hairline--top">
      ...
      <div class="stock-item">stock {{ basicInfo.stores }}</div>
    </div>
    <Coupons title="领券" />
<!--    <van-cell>-->
<!--      <template #title>-->
<!--        <div class="cell-bar">-->
<!--          <div class="cell-bar-title">Guarantee</div>-->
...
<!--    </van-cell>-->
    ...
    <Plate title="Details" class="mt10" />
    <div class="goods-content" v-html="content"></div>
    <div class="action-bar-perch"></div>
    <!-- 商品导航栏 -->
    <van-action-bar>
      <van-action-bar-icon icon="thumb-circle-o" text="Home" to="/home" replace />
      <van-action-bar-icon icon="chat-o" text="Support" @click="onConcatService" />
      <van-action-bar-icon icon="cart-o" text="Cart" to="/cart" :badge="cartCount" replace />
      <van-action-bar-button type="warning" text="Add" @click="onSkuShow('addCart')" />
      <van-action-bar-button type="danger" text="Buy" @click="onSkuShow" />
    </van-action-bar>
    <!-- SKU 弹窗 -->
