...
<!--          <div class="empty-title">购物车快饿瘪了 T.T</div>-->
          <div class="empty-txt">No Item</div>
          <van-button class="empty-btn" round plain type="primary" @click="goHome">Go shopping</van-button>
        </template>
        <template v-else>
<!--          <div class="empty-title">登录后才能看到您的购物车</div>-->
          <van-button class="empty-btn" type="primary" round @click="goLogin">Login</van-button>
        </template>
      </van-empty>
    </SpainList>
    <!--结算栏 -->
      ...
      <div v-if="list.length" class="submit-bar">
        ...
        <template v-else>
          <div class="submit-bar-hd">
            <span class="submit-bar-total">Total：</span>
          ...
          <van-button class="submit-bar-button" round type="primary" @click="onSubmit"
            >Payment({{ totalGoodCount }})</van-button
          >
        </template>
      </div>
    </div>
